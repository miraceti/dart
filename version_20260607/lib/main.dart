    ...
    : SingleChildScrollView(
        child: Column(
          children: [
            ...
            // 📋 TABLE EXOPLANÈTES LES PLUS PROCHES
            Container(
              margin: const EdgeInsets.all(12),
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
              ...
              child: Column(
                ...
                children: [
                  ...
                  FutureBuilder<List<dynamic>>(
                    future: NasaApi.fetchClosestPlanets(),
                    builder: (context, snapshot) {
                      // 👇 AJOUTE ICI LES PRINTS
                      print(snapshot.data);
                      ...
                      final List planets = (snapshot.data as List<dynamic>? ?? [])
                      ...
                      return SingleChildScrollView(
                        ...
                        child: DataTable(
                          columns: const [
                            DataColumn(label: Text("Exoplanète")),
                            DataColumn(label: Text("Distance (AL)")),
                          ],
                          rows: planets.map((p) {
                            final name = (p['pl_name'] ?? 'Unknown').toString();

                            final parsec = (p['sy_dist'] ?? 0);
                            final distanceLy = (parsec is num) ? parsec * 3.26156 : 0.0;

                            return DataRow(cells: [
                              DataCell(Text(name)),
                              DataCell(Text(distanceLy.toStringAsFixed(2))),
                            ]);
                          }).toList(),
                        ),
                      );
